...
- Integrated only matching directional diffuse terms into Enhanced per-pixel lighting using call/depth receiver validation. Ambient/fog/combiner, Native, raster linkage and336-byte RDPParams remain unchanged; FramebufferParams now48 bytes. No legacy RT_ENABLED, AO/denoising/reflections/GI work.
- Build and actual DXIL/SPIR-V/wrappers passed; lib_6_5 is required for GeometryIndex. Fixed a user-observed vertically flipped inset by accounting for both existing RSPProcessCS/RasterVS Y negations; production mask shares that mapping.
- Root directly verified Vulkan RX9070XT Town/saveA primary, visible/occluded sunlight, production shadows and OFF. Real PNGs/logs/build evidence in `_working-directory/diagnostics/2026-09-10-rt-shadows/`; static occluded samples darken while sunlit samples remain unchanged. No severe visible regression/device loss. Captures are not animation-frame-matched; D3D12 runtime/MSAA/broad qualification not performed.
- After checkpoint: added persistent default-OFF Graphics RT Sun Shadows + independent F1 session toggle, with one N64ModernRuntime GraphicsConfig field and existing parent persistence/binding. Kept the added row reachable through local scroll overflow. User manually verified Graphics/F1 and closed the runtime; no more UI/runtime tests requested. Secondary eligibility now rejects inward/asymmetric custom clipping.
- Preserved known-good pre-final binary/evidence and six user screenshots, then ran the final normal project build successfully. Final SHA `D8DF2EF071240676433F8DD02D14BE70B4CD3DF9651A8D645D85D4121F685CD8`; executable/assets synchronized at normal build output and existing isolated runtime. Archive/log/hashes in `finished-run2/`. No runtime repeat after successful final build.
- Document-only unresolved reports: sunrise appearance/disappearance before stability (MM time/skybox/light-color/sun-elevation disagreement is a user-reported plausible explanation, not diagnosed); separately temporary shadow collapse when camera enters/backs into geometry. Many interiors/open-roof cases still need richer lighting/coverage. Do not conflate these issues or claim they were fixed.
- No new ADR required: implementation extends the existing semantic-lighting/Native/RT foundation decisions. Forward-looking art-direction synthesis follows in `docs/RT_LIGHTING_VISION.md`; no next feature implemented. Generated CHANGELOG.md remains under its release-note workflow; this session is recorded here.
- Checkpoint candidate SHA `DD9CE9427BE7BDBEB129E717A03B9FF3B5457D9FB27E556EA92FC5450792474F`. Stop/wait/exclusive launcher fixes copied-profile overlap after denied sandbox process termination. All test processes stopped and copied seed restored. Source profile untouched; submodules preserved. Updated HANDOFF/foundation before bounded UI work.
